...
class TestFlowchart:
    ...
    def test_svg_renders(self):
        ...
        assert "CONSORT Participant Flow Diagram" in svg
        ...
        assert "B (n = 4)" in svg
        assert "PFS Multivariable (n = 4)" in svg  # two-line analyzed subtext
    ...
    def test_exclusion_with_nan_covariate(self):
        ...
        svg = Path(out).read_text()
        assert "PFS Multivariable (n = 3)" in svg
        assert "PFS Multivariable (n = 2)" in svg
